...
impl<'a> ObjectRayIntersection<'a> {
    ...
    pub fn check_intersection(
        ...
        for object in objects {
            if let Some(t) = object.is_object_hit(&ray) {
        ...
        for light in lights {
            if let Some(light_object) = light.get_object() {
                if let Some(t) = light_object.is_object_hit(&ray) {
                    if t < smallest_t && t > min_t && t < max_t {
                        smallest_t = t;
                        is_light_hit = true;
        ...
        hit_object.map(|hit_object| Self::new(ray, smallest_t, hit_object, is_light_hit))
    }
    ...
    pub fn create_object(object: Objects) -> Box<dyn Object> {
        match object {
            Objects::Sphere(position, radius, material) => {
                Box::new(Sphere::new(position, radius, material))
            }
            Objects::Triangle(vertice_1, vertice_2, vertice_3, material) => {
                Box::new(Triangle::new(vertice_1, vertice_2, vertice_3, material))
            }
            Objects::Panel(panel_origin, panel_width, panel_height, panel_normal, material) => {
                Box::new(Panel::new(
                    panel_origin,
                    panel_width,
                    panel_height,
